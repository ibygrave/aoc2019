import pytest
from aoc2019 import intcode


@pytest.mark.parametrize("program,out", [
    ("1,0,0,0,99", "2,0,0,0,99"),
    ("2,3,0,3,99", "2,3,0,6,99"),
    ("2,4,4,5,99,0", "2,4,4,5,99,9801"),
    ("1,1,1,4,99,5,6,0,99", "30,1,1,4,2,5,6,0,99"),
    ("1,12,2,3,1,1,2,3,1,3,4,3,1,5,0,3,2,1,6,19,1,19,6,23,2,23,6,27,2,"
     "6,27,31,2,13,31,35,1,9,35,39,2,10,39,43,1,6,43,47,1,13,47,51,2,6,"
     "51,55,2,55,6,59,1,59,5,63,2,9,63,67,1,5,67,71,2,10,71,75,1,6,75,"
     "79,1,79,5,83,2,83,10,87,1,9,87,91,1,5,91,95,1,95,6,99,2,10,99,103,"
     "1,5,103,107,1,107,6,111,1,5,111,115,2,115,6,119,1,119,6,123,1,123,"
     "10,127,1,127,13,131,1,131,2,135,1,135,5,0,99,2,14,0,0",
     "3224742,12,2,2,1,1,2,3,1,3,4,3,1,5,0,3,2,1,6,24,1,19,6,26,2,23,6,"
     "52,2,6,27,104,2,13,31,520,1,9,35,523,2,10,39,2092,1,6,43,2094,1,13,"
     "47,2099,2,6,51,4198,2,55,6,8396,1,59,5,8397,2,9,63,25191,1,5,67,"
     "25192,2,10,71,100768,1,6,75,100770,1,79,5,100771,2,83,10,403084,"
     "1,9,87,403087,1,5,91,403088,1,95,6,403090,2,10,99,1612360,1,5,103,"
     "1612361,1,107,6,1612363,1,5,111,1612364,2,115,6,3224728,1,119,6,"
     "3224730,1,123,10,3224734,1,127,13,3224739,1,131,2,3224741,1,135,5,"
     "0,99,2,14,0,0"),
    ("1002,4,3,4,33", "1002,4,3,4,99"),
    ("1101,100,-1,4,0", "1101,100,-1,4,99"),
    ])
def test_program_a(program, out):
    prog = intcode.Program(program)
    assert program == str(prog)
    assert [] == list(prog)
    assert out == str(prog)


@pytest.mark.parametrize("program,in_data,out_data", [
    ("3,0,4,0,99", [3141], [3141]),
    ("3,9,8,9,10,9,4,9,99,-1,8", [8], [1]),
    ("3,9,8,9,10,9,4,9,99,-1,8", [5], [0]),
    ("3,9,7,9,10,9,4,9,99,-1,8", [7], [1]),
    ("3,9,7,9,10,9,4,9,99,-1,8", [8], [0]),
    ("3,9,7,9,10,9,4,9,99,-1,8", [9], [0]),
    ("3,3,1108,-1,8,3,4,3,99", [8], [1]),
    ("3,3,1108,-1,8,3,4,3,99", [1], [0]),
    ("3,3,1107,-1,8,3,4,3,99", [0], [1]),
    ("3,3,1107,-1,8,3,4,3,99", [10], [0]),
    ("3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9", [0], [0]),
    ("3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9", [100], [1]),
    ("3,3,1105,-1,9,1101,0,0,12,4,12,99,1", [0], [0]),
    ("3,3,1105,-1,9,1101,0,0,12,4,12,99,1", [-1], [1]),
    ("3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,"
     "0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,20,"
     "4,20,1105,1,46,98,99", [4], [999]),
    ("3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,"
     "0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,"
     "20,4,20,1105,1,46,98,99", [8], [1000]),
    ("3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,"
     "0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,"
     "20,4,20,1105,1,46,98,99", [9], [1001]),
    ("109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99", [],
        [109, 1, 204, -1, 1001, 100, 1, 100, 1008,
         100, 16, 101, 1006, 101, 0, 99]),
    ("1102,34915192,34915192,7,4,7,99,0", [], [34915192**2]),
    ("104,1125899906842624,99", [], [1125899906842624]),
    ])
def test_program_b(program, in_data, out_data):
    prog = intcode.Program(program)
    prog.set_input(in_data)
    assert program == str(prog)
    prog_out_data = list(prog)
    assert out_data == prog_out_data


AMP_CONTROL_TESTS = [
    ("3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0",
     [4, 3, 2, 1, 0], 43210, intcode.control_amps),
    ("3,23,3,24,1002,24,10,24,1002,23,-1,23,101,5,23,"
     "23,1,24,23,23,4,23,99,0,0",
     [0, 1, 2, 3, 4], 54321, intcode.control_amps),
    ("3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,1002,33,7,33,"
     "1,33,31,31,1,32,31,31,4,31,99,0,0,0",
     [1, 0, 4, 3, 2], 65210, intcode.control_amps),
    ("3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26,27,4,27,1001,28,-1,"
     "28,1005,28,6,99,0,0,5",
     [9, 8, 7, 6, 5], 139629729, intcode.feedback_control_amps),
    ("3,52,1001,52,-5,52,3,53,1,52,56,54,1007,54,5,55,1005,55,26,1001,"
     "54,-5,54,1105,1,12,1,53,54,53,1008,54,0,55,1001,55,1,55,2,53,55,"
     "53,4,53,1001,56,-1,56,1005,56,6,99,0,0,0,0,10",
     [9, 7, 8, 5, 6], 18216, intcode.feedback_control_amps),
    ]


@pytest.mark.parametrize("program, phases, signal_out, control_fn",
                         AMP_CONTROL_TESTS)
def test_control_amps(program, phases, signal_out, control_fn):
    assert signal_out == control_fn(program, phases)


@pytest.mark.parametrize("program, phases, signal_out, control_fn",
                         AMP_CONTROL_TESTS)
def test_optimize_amps(program, phases, signal_out, control_fn):
    phases_in = phases[:]
    phases_in.sort()
    best_amp_control = intcode.optimize_amps(program, phases_in, control_fn)
    assert signal_out == best_amp_control.signal
    assert phases == best_amp_control.phases
