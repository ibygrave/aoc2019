...
@pytest.mark.parametrize("program, phases, signal_out", AMP_CONTROL_TESTS)
def test_control_amps(program, phases, signal_out):
    assert signal_out == aocintcode.control_amps(program, phases)


@pytest.mark.parametrize("program, phases, signal_out", AMP_CONTROL_TESTS)
def test_optimize_amps(program, phases, signal_out):
    best_amp_control = aocintcode.optimize_amps(program, list(range(len(phases))))
    assert signal_out == best_amp_control.signal
    assert phases == best_amp_control.phases


FEEDBACK_AMP_CONTROL_TESTS = [
...
@pytest.mark.parametrize("program, phases, signal_out", FEEDBACK_AMP_CONTROL_TESTS)
def test_feedback_control_amps(program, phases, signal_out):
    assert signal_out == aocintcode.feedback_control_amps(program, phases)
